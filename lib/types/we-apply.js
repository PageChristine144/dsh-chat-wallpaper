 ...
 */
export function applyWeProject(engineDir, projectDir, seam = defaultWeApplySeam) {
    const projectJson = join(projectDir, 'project.json');
    if (!existsSync(projectJson))
        return { ok: false, reason: 'project-json-missing' };
    ...
    if (result.status !== 0)
        return { ok: false, reason: `control-failed:${String(result.status)}` };
    return { ok: true };
}
/**
 ...
 */
export function setWeAudio(engineDir, muted, seam = defaultWeApplySeam) {
    if (!existsSync(join(engineDir, 'wallpaper64.exe')) && !existsSync(join(engineDir, 'wallpaper32.exe'))) {
        return { ok: false, reason: 'engine-not-found' };
    }
    if (!seam.isWeRunning())
...
//# sourceMappingURL=we-apply.js.map