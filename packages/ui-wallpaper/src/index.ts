...
// Type-only: pulls the ctx.webServer merge into this program.
import type {} from '@deepseek-ai/dsh-host-webserver'
import { WALLPAPER_SETTINGS_NAMESPACE, WallpaperSettingsSchema } from './wallpaper-settings.ts'
import { closeShell, openShell, type ChatDesktopConfig } from './chat-desktop-host.ts'
import {
  defaultProjectRoots, defaultWorkshopRoots, resolveEngineRoot, resolveProjectDir, resolveRaw, scanWallpapers, WE_MIME,
} from './we-host.ts'
import { applyWeProject, setWeAudio } from './we-apply.ts'

export {
  DEFAULT_WALLPAPER_SETTINGS, WALLPAPER_MODES, WALLPAPER_SETTINGS_FIELDS, WALLPAPER_SETTINGS_NAMESPACE,
  type WallpaperMode, type WallpaperSettings,
} from './wallpaper-settings.ts'
export type { WeListItem } from './we-host.ts'
export {
  decodeProjectJson, defaultProjectRoots, defaultWorkshopRoots, resolveEngineRoot, resolveProjectDir, resolveRaw, scanWallpapers,
} from './we-host.ts'
...
export function apply(
  ctx: Context,
  config: Config = { workshopRoots: DEFAULT_WORKSHOP_ROOTS, projectRoots: DEFAULT_PROJECT_ROOTS },
): void {
  ...
  ctx.inject(['webServer'], (httpCtx) => {
    ...
    const resolveRoot = (kind: string): string | undefined =>
      kind === 'workshop' ? workshopRoots.find(exists) : projectRoots.find(exists)

    /** GET/HEAD /wallpaper-engine/list → the browseable library as JSON. */
    const serveList = (req: IncomingMessage, res: ServerResponse): void => {
    ...
    const serveRaw = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      if (req.method !== 'GET' && req.method !== 'HEAD') {
        res.writeHead(405)
        res.end()
        return
    ...
    httpCtx.effect(
      () => httpCtx.webServer.register({ kind: 'exact', path: '/wallpaper-engine/list', handler: serveList }),
      'ui-wallpaper: Wallpaper Engine list route',
    )
    httpCtx.effect(
      () => httpCtx.webServer.register({ kind: 'prefix', path: '/wallpaper-engine/raw', handler: serveRaw }),
      'ui-wallpaper: Wallpaper Engine raw route',
    )

    /** POST /wallpaper-engine/apply → switch the live desktop wallpaper through WE. */
    const serveWeApply = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      if (req.method !== 'POST') {
        res.writeHead(405)
        res.end()
        return
      ...
      try {
        ...
        if (typeof body === 'object' && body !== null && typeof (body as { key?: unknown }).key === 'string') {
    ...
    httpCtx.effect(
      () => httpCtx.webServer.register({ kind: 'exact', path: '/wallpaper-engine/apply', handler: serveWeApply }),
      'ui-wallpaper: Wallpaper Engine apply route',
    )

    /** POST /wallpaper-engine/audio → mute or unmute all WE wallpapers. */
    const serveWeAudio = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      if (req.method !== 'POST') {
        res.writeHead(405)
        res.end()
        return
      ...
      try {
        ...
        if (typeof body === 'object' && body !== null && typeof (body as { muted?: unknown }).muted === 'boolean') {
          muted = (body as { muted: boolean }).muted
        }
      } catch {
    ...
    httpCtx.effect(
      () => httpCtx.webServer.register({ kind: 'exact', path: '/wallpaper-engine/audio', handler: serveWeAudio }),
      'ui-wallpaper: Wallpaper Engine audio route',
    )

    /** POST /chat-desktop/open → spawn the transparent shell (idempotent). */
    const serveDesktopOpen = (req: IncomingMessage, res: ServerResponse): void => {
      if (req.method !== 'POST') {
        res.writeHead(405)
        res.end()
        return
    ...
    const serveDesktopClose = (req: IncomingMessage, res: ServerResponse): void => {
      if (req.method !== 'POST') {
        res.writeHead(405)
        res.end()
        return
    ...
    httpCtx.effect(
      () => httpCtx.webServer.register({ kind: 'exact', path: '/chat-desktop/open', handler: serveDesktopOpen }),
      'ui-wallpaper: transparent desktop open route',
    )
    httpCtx.effect(
      () => httpCtx.webServer.register({ kind: 'exact', path: '/chat-desktop/close', handler: serveDesktopClose }),
 ...
 */
function streamRange(path: string, start: number, end: number, res: ServerResponse): Promise<void> {
